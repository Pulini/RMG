...
public class WheelView extends View {
    ...
    public static final int FONT_COLOR = Color.parseColor("#029DF9");
...
//    private String selectTip;
    ...
    public void init(Context context,AttributeSet attrs){
        ...
        fontSize = obtainStyledAttributes.getDimensionPixelSize(R.styleable.WheelView_textTimeSize, FONT_SIZE);
    }
...
//    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //初始化WheelSelect
        wheelSelect = new WheelSelect(showCount / 2 * itemHeight, width, itemHeight, "秒", fontColor, fontSize, padding);
        //得到所有的高度
    ...
    private void initWheelItems(int width, int itemHeight) {
        ...
        for (int i = 0; i < showCount + 2; i++) {
            ...
            Log.e("bcz","stringIndex:" + stringIndex + "===lists.size:" + lists.size());
            if (stringIndex >= lists.size()) {
                //stringIndex = lists.size()-1;
                select = select - lists.size();
//                continue;
                continue;
            }
            wheelItems.add(new WheelItem(startY, width, itemHeight, fontColor, fontSize, lists.get(stringIndex),showCount ));
        }
    }

    private void initWheelItems2(int width, int itemHeight) {
        wheelItems.clear();
        for (int i = 0; i < showCount + 2; i++) {
            int startY = itemHeight * (i - 1);
            int stringIndex = select - showCount / 2 - 1 + i;
            ...
            if (stringIndex >= lists.size()) {
                //stringIndex = lists.size()-1;
                select = select - lists.size();
                wheelItems.add(new WheelItem(startY, width, itemHeight, fontColor, fontSize, lists.get(stringIndex - lists.size()),showCount ));
//                continue;
//                continue;
            }else{
                wheelItems.add(new WheelItem(startY, width, itemHeight, fontColor, fontSize, lists.get(stringIndex),showCount ));
    ...
    public void setSelectItem(int index){
        select = index;
        initWheelItems2(width, itemHeight);
